...
#else
    /* File is neither a directory nor an HDF5 file nor S3 file -> return an error */
        if (rc_open != NULL) *rc_open = TREXIO_FILE_ERROR;
        return NULL;
#endif
      }
...
#else
    /* In the current implementation if HDF5 back end is not available - then there is two backs end left */
#ifdef HAVE_S3
    trexio_exit_code rc_text=trexio_text_inquire(file_name);
    if ( rc_text == TREXIO_SUCCESS){
        back_end_local = TREXIO_TEXT;
    }else {
        trexio_exit_code rc_3=trexio_s3_inquire(file_name);
        if ( rc_s3 == TREXIO_SUCCESS){
            back_end_local=TREXIO_S3;
        } else {
            if (rc_open != NULL)*rc_open=TREXIO_FILE_ERROR;

...
** Back ends
   ...
   Then the corresponding back-end ~has/read/write~ functions has to be implemented.
...
*** Fortran
  ...
  integer(trexio_back_end_t), parameter :: TREXIO_S3 = 2
  integer(trexio_back_end_t), parameter :: TREXIO_INVALID_BACK_END = 3
...
class File:
    ...
    back_end: int
        ...
        For example, TREXIO_HDF5 (0) or TREXIO_TEXT (1) or TREXIO_S3 (3).
    mode: str
...
** File opening
   ...
   **Input parameters:**
     ...
     3) ~back_end~      - integer number (or the corresponding global parameter) specifying the back end
       ...
       - ~TREXIO_S3~ - for TEXT back end (integer alternative: 2)
            ...
            const back_end_t back_end, trexio_exit_code* const rc_open)
{
  ...
  if (back_end == TREXIO_AUTO && mode == 'r') {
    ...
    rc = trexio_text_inquire(file_name);
    if (rc == TREXIO_SUCCESS) {
...
#ifdef HAVE_HDF5
  if (back_end_local == TREXIO_AUTO && mode == 'r') {
    /* Check if the TREXIO file is an HDF5 file */
    rc = trexio_hdf5_inquire(file_name);
    if (rc == TREXIO_SUCCESS) {
...
#ifdef HAVE_S3
  if (back_end_local == TREXIO_AUTO && mode == 'r') {
    /* Check if the TREXIO file is an S3 file */
    rc = trexio_s3_inquire(file_name);
    if (rc == TREXIO_SUCCESS) {
      back_end_local = TREXIO_S3;
    }
  }
#endif
...
#ifdef HAVE_S3
  case TREXIO_S3:
    result_tmp = malloc(sizeof(trexio_s3_t));
    break;
#endif
...
printf("-front-code_rc_has_metadata_package_version=%d\n",rc);///////////
  ...
  /* Write TREXIO_PACKAGE_VERSION upon creation of the file */
...
#ifdef HAVE_S3
    case TREXIO_S3:
      rc = trexio_s3_write_metadata_package_version(result, TREXIO_PACKAGE_VERSION);
      break;
#endif
    ...
    if (rc != TREXIO_SUCCESS) {
printf("-front- PROBLEME : trexio_s3_write_metadata_package_version\n");
...
#ifdef HAVE_S3
    case TREXIO_S3:
      rc = trexio_s3_read_metadata_package_version(result, version_origin, 16);
      break;
#endif
...
** File existence
    ...
    - ~TREXIO_SUCCESS~ if the ~file_name~ exists and if it correspond to a valid TREXIO file (i.e. a directory OR an HDF5 file OR a S3 file)
...
#if defined(HAVE_HDF5) || defined(HAVE_S3)
 /*If it's not a valid TEXT file, continue testing the other backends */
  ...
  if(rc_hdf5==TREXIO_SUCCESS) return rc_hdf5;
#endif
#ifdef HAVE_S3
  trexio_exit_code rc_s3=trexio_s3_inquire(file_name);
  if(rc_s3==TREXIO_SUCCESS) return rc_s3;
#endif
...
#else
/*If neither HDF5 nor S3 is compiled, return what TEXT returns */
  return rc_text;
#endif
}
    #+end_src
...
trexio_cp(const char* source, const char* dest)
{
  ...
  /* Try to determine the applicable backend if the back_end argument is TREXIO_AUTO */
  /* Check if the TREXIO file exists and if it is a directory */
  trexio_exit_code rc = trexio_text_inquire(source);
  if (rc == TREXIO_SUCCESS) {
     back_end_local = TREXIO_TEXT;
  }
#ifdef HAVE_HDF5
...
#ifdef HAVE_S3
  if (rc != TREXIO_SUCCESS) {
 	/* If not, check if it is an S3 file */
 	rc = trexio_s3_inquire(source);
 	if (rc == TREXIO_SUCCESS) {
 	   back_end_local = TREXIO_S3;
 	}
  }
#endif
  if (rc != TREXIO_SUCCESS) {
    /* File is neither a directory nor an HDF5 file nor a S3 file -> return an error */
    return TREXIO_FILE_ERROR;
  }
...
trexio_has_$group$ (trexio_t* const file)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_has_$group$(file);
#else
    return TREXIO_BACK_END_MISSING;
#endif
/*
...
trexio_read_$group_num$_64 (trexio_t* const file, $group_num_dtype_double$* const num)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_$group_num$(file, num);
#else
    rc = TREXIO_BACK_END_MISSING;
#endif
    break;
...
trexio_write_$group_num$_64 (trexio_t* const file, const $group_num_dtype_double$ num)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_write_$group_num$(file, num_write);
#else
    return TREXIO_BACK_END_MISSING;
#endif
...
trexio_read_$group_num$_32 (trexio_t* const file, $group_num_dtype_single$* const num)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_$group_num$(file, &num_64);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break ;
...
trexio_write_$group_num$_32 (trexio_t* const file, const $group_num_dtype_single$ num)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_write_$group_num$(file, ($group_num_dtype_double$) num_write);
#else
    return TREXIO_BACK_END_MISSING;
#endif
/*
...
trexio_has_$group_num$ (trexio_t* const file)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_has_$group_num$(file);
#else
    return TREXIO_BACK_END_MISSING;
#endif
/*
...
trexio_read_$group_dset$_64 (trexio_t* const file, $group_dset_dtype_double$* const $group_dset$)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_$group_dset$(file, $group_dset$, rank, dims);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break;
...
trexio_write_$group_dset$_64 (trexio_t* const file, const $group_dset_dtype_double$* $group_dset$)
{
  ...
  if ($is_index$) {
    ...
    case TREXIO_S3:
#ifdef HAVE_S3
      rc = trexio_s3_write_$group_dset$(file,
                                          (const $group_dset_dtype_double$*) $group_dset$_p,
                                          rank, dims);
      break;
#else
    ...
    case TREXIO_S3:
#ifdef HAVE_S3
      rc = trexio_s3_write_$group_dset$(file, $group_dset$, rank, dims);
      break;
#else
      rc = TREXIO_BACK_END_MISSING;
      break;
...
trexio_read_$group_dset$_32 (trexio_t* const file, $group_dset_dtype_single$* const $group_dset$)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_$group_dset$(file, $group_dset$_64, rank, dims);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break;
...
trexio_write_$group_dset$_32 (trexio_t* const file, const $group_dset_dtype_single$* $group_dset$)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_write_$group_dset$(file, $group_dset$_64, rank, dims);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break;
...
trexio_has_$group_dset$ (trexio_t* const file)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_has_$group_dset$(file);
#else
    return TREXIO_BACK_END_MISSING;
#endif
/*
...
#undef unique_rank
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_$group_dset$(file, offset_file, *buffer_size, max_dim, &eof_read_size, index_sparse, value_sparse);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break;
...
trexio_read_$group_dset$_size(trexio_t* const file, int64_t* const size_max)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_read_$group_dset$_size(file, size_max);
    break;
#else
    return TREXIO_BACK_END_MISSING;
#endif
...
#undef unique_rank
  ...
  if (file->one_based) {
    ...
    case TREXIO_S3:
#ifdef HAVE_S3
      rc = trexio_s3_write_$group_dset$(file, offset_file, buffer_size, max_dim,
                                          size_max,index_sparse_p, value_sparse);
      break;
#else
      rc = TREXIO_BACK_END_MISSING;
...
trexio_read_$group_dset$_low (trexio_t* const file, char* dset_out, const int32_t max_str_len)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_read_$group_dset$(file, dset_out, rank, dims, (uint32_t) max_str_len);
#else
    return TREXIO_BACK_END_MISSING;
#endif
/*
...
trexio_write_$group_dset$_low (trexio_t* const file, char* dset_in, const int32_t max_str_len)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_write_$group_dset$(file, (const char**) dset_str, rank, dims);
    break;
#else
    rc =TREXIO_BACK_END_MISSING;
    break;
...
trexio_read_$group_dset$ (trexio_t* const file, const int64_t offset_file, int64_t* const buffer_size_read, double* const dset)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_$group_dset$(file, offset_file, rank, dims, &eof_read_size, dset);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break;
...
trexio_write_$group_dset$ (trexio_t* const file, const int64_t offset_file, const int64_t buffer_size, const double* dset)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_write_$group_dset$(file, offset_file, rank, dims, dset);
    break;
#else
    return TREXIO_BACK_END_MISSING;
    break;
...
trexio_read_$group_str$ (trexio_t* const file, char* const str_out, const int32_t max_str_len)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_read_$group_str$(file, str_out, (uint32_t) max_str_len);
#else
    return TREXIO_BACK_END_MISSING;
#endif
...
trexio_write_$group_str$ (trexio_t* const file, const char* str, const int32_t max_str_len)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_write_$group_str$(file, str);
#else
    return TREXIO_BACK_END_MISSING;
#endif
...
trexio_has_$group_str$ (trexio_t* const file)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_has_$group_str$(file);
#else
    return TREXIO_BACK_END_MISSING;
#endif
...
trexio_delete_$group$ (trexio_t* const file)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_delete_$group$(file);
#else
    return TREXIO_BACK_END_MISSING;
#endif
...
trexio_read_determinant_list (trexio_t* const file, const int64_t offset_file, int64_t* const buffer_size_read, int64_t* const dset)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_read_determinant_list(file, offset_file, rank, dims, &eof_read_size, dset);
    break;
#else
    rc = TREXIO_BACK_END_MISSING;
    break;
...
trexio_write_determinant_list (trexio_t* const file, const int64_t offset_file, const int64_t buffer_size, const int64_t* dset)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    rc = trexio_s3_write_determinant_list(file, offset_file, rank, dims, dset);
    break;
#else
    return TREXIO_BACK_END_MISSING;
    break;
...
trexio_has_determinant_list (trexio_t* const file)
{
  ...
  case TREXIO_S3:
#ifdef HAVE_S3
    return trexio_s3_has_determinant_list(file);
#else
    return TREXIO_BACK_END_MISSING;
#endif

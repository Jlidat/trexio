...
bool trexio_s3_file_exists(const char* file_name);
//bool trexio_s3_file_exists(trexio_s3_client* client, const char* file_name);
    #+end_src

    #+begin_src c :tangle basic_s3.c
bool
//trexio_s3_file_exists (trexio_s3_client* client, const char* file_name)
trexio_s3_file_exists (const char* file_name)
{
  /* Check if the client and the file with "file_name" exists*/
/*        if(client==NULL || file_name==NULL){
                return false;  //Défense contre les pointeurs NULL
        }
        int32_t exists=s3_file_exists(client, file_name, strlen(file_name));
        return exists==0;
*/
}
    #+end_src
...
//!!      bool file_exists = trexio_s3_file_exists(client, file_name);
      s3_disconnect(client);
//!!      return file_exists ? TREXIO_SUCCESS : TREXIO_FAILURE;
...
trexio_s3_init (trexio_t* const file)
{
 ...
 f->client=s3_connect();
  ...
  bool exist;
  exist = s3_file_exists(f->client, file->file_name, strlen(file->file_name));
...
trexio_s3_has_$group$ (trexio_t* const file)
{
  ...
  if (file->mode != 'r') {
    trexio_exit_code rc = trexio_s3_flush_$group$((trexio_s3_t*) file);
  ...
  bool file_exists;
//  file_exists = trexio_s3_file_exists(file->client, $group$_full_path);
  file_exists = trexio_s3_file_exists($group$_full_path);
...
//=== Upload final vers S3 ===
  int rc=s3_put(file->client, s3_path, buffer, buffer_size);
  FREE(buffer);
                                                ...
                                                const double* value_sparse)
{
  ...
  bool file_exists = trexio_s3_file_exists(file_full_path);
//  bool file_exists = trexio_s3_file_exists(file->client, file_full_path);

...
trexio_exit_code trexio_s3_flush(trexio_t* const file);
    ...
    To check that the TREXIO file exists, we need to check if there is
    a key with the file name as prefix. As all TREXIO files contain
    metadata, we check for the =metadata.txt= file.
    
    #+begin_src c :tangle basic_s3.c
static int file_exists(trexio_s3_client* client, const char* file_name)
{
      assert(client != NULL);
      assert(file_name != NULL);

      const char * metadata_txt = "/metadata.txt";

      size_t len = strlen(file_name) + 1; // Size of file_name + NULL char
      size_t len2 = len+strlen(metadata_txt);  // Size of file_name + "/metadata.txt" + NULL char

      // Temporary string for file_name + "/metadata.txt"
      char *key = malloc(sizeof(char) * len2);
      if (key == NULL) {
        return TREXIO_ALLOCATION_FAILED;
      }
      memcpy(key, file_name, len);
      key = strncat(key, metadata_txt, len2);

      int exists = s3_file_exists(client, key, strlen(key));
...
trexio_s3_inquire (const char* file_name)
{
      ...
      int exists = file_exists(client, file_name);
...
trexio_s3_init (trexio_t* const file)
{
  ...
  int exists = file_exists(f->client, file_name);

 /* If file does not exist - create it in write mode */
  if( (exists != 0) && (file->mode == 'r') ) {
...
{
printf("===== Début de trexio_s3_flush ======\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
printf("===== Début de trexio_s3_read_group ======\n");
  ...
  int64_t sz=s3_size(file->client, $group$->file_name, strlen($group$->file_name));
  if(sz > 0){
    ...
    int rc2;
    rc = TREXIO_FAILURE;
    while(sscanf(line, "%1023s%n", buffer, &nread) != EOF) {
//        line += nread;
       printf(":Dline1:%s:Fline1:\n", line);
        printf("==> Mot détecté (buffer1): %s, %ld\n\n", buffer, strlen(buffer));
        line += nread;
        printf(":Dline1-bis:%s:Fline1-bis:\n", line);
        
	if (strcmp(buffer, "EXIT") == 0) {
		printf("==> Traitement de 'exit'\n");
                rc = TREXIO_SUCCESS;
       		break;
        // START REPEAT GROUP_DSET_ALL
        } else if (strcmp(buffer, "rank_$group_dset$") == 0) {
		printf("==> Traitement de 'rank_$group_dset$'\n");

        	rc2 = sscanf(line, "%u\n%n", &($group$->rank_$group_dset$), &nread);
               printf(":Dline2:%s:Fline2:\n", line);
                printf("==> Résultat sscanf : rc2 = %d\n", rc2);
                printf("==> Valeur extraite :  %u\n", $group$->rank_$group_dset$);
                printf("==> Nombre de caractères lus : nread = %d\n", nread);
                printf("==> Résultat : %s=%u\n\n", buffer, $group$->rank_$group_dset$);

        	line += nread;
		printf(":Dline2-bis:%s:Fline2-bis:\n", line);
        	if (rc2 != 1) {
        	...
        	for (uint32_t i=0; i<$group$->rank_$group_dset$; ++i){
        		uint32_t j=0;
        		rc2 = sscanf(line, "%1023s %u\n%n", buffer, &j, &nread);
			printf(":Dline3:%s:Fline3:\n", line);
                        printf("==> Mot détecté (buffer3): %s, %ld\n\n", buffer, strlen(buffer));

	       		line += nread;
			printf(":Dline3-bis:%s:Fline3-bis:\n", line);
         		if ((rc2 != 2) || (strcmp(buffer, "dims_$group_dset$") != 0) || (j!=i)) {
				printf("==> Traitement de 'dims_$group_dset$'\n");
            			trexio_s3_free_read_$group$(buffer, f, file, $group$);
            			return NULL;
          		}
          		rc2 = sscanf(line, "%" SCNu64 "\n%n", &($group$->dims_$group_dset$[i]), &nread);

			printf(":Dline4:%s:Fline4:\n", line);
			printf("==> Résultat sscanf : rc2 = %d\n", rc2);
printf("==> Valeur extraite : %" PRIu64 "\n", $group$->dims_$group_dset$[i]);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
//      printf("==> Résultat : %s=%" PRIu64 "\n%n", buffer, $group$->dims_$group_dset$[i]);

			line += nread;
			printf(":Dline4-bis:%s:Fline4-bis:\n", line);
	        	assert(rc2 == 1);
          		if (rc2 != 1) {
           			trexio_s3_free_read_$group$(buffer, f, file, $group$);
            			return NULL;
        ...
        // START REPEAT GROUP_DSET_NUM
		 ...
		 printf("==> Traitement de '$group_dset$'\n");
        /* Allocate arrays */
        	...
        	for (uint64_t i=0 ; i<size_$group_dset$ ; ++i) {
          		rc2 = sscanf(line, "%$group_dset_format_scanf$%n", &($group$->$group_dset$[i]), &nread);
printf(":Dline5:%s:Fline5:\n", line);
printf("==> Résultat sscanf : rc2 = %d\n", rc2);
//printf("==> Valeur extraite : $group$->$group_dset$[%ld] = %PRId64\n", i, $group$->$group_dset$[i]);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
//      printf("==> Résultats: %s, %$group_dset_format_scanf$%n\n", buffer, $group$->$group_dset$[i]);
	  		line += nread;
                        printf(":Dline5:%s:Fline5:\n", line);
          		if (rc2 != 1) {
            			trexio_s3_free_read_$group$(buffer, f, file, $group$);
        ...
        // START REPEAT GROUP_DSET_STR
          ...
          /* Allocate arrays */
printf("DEBUG: allocation de $group_dset$ pour %lu chaines\n", size_$group_dset$);
                        $group$->$group_dset$ = CALLOC(size_$group_dset$, $group_dset_dtype$);
          ...
          /* WARNING: this tmp array allows to avoid allocation of space for each element of array of string
           ...
           ,*/
			...
			FREE($group$->$group_dset$); ////
                        ...
                        for (uint64_t i=0 ; i<size_$group_dset$ ; ++i) {
//				$group_dset$[i]=CALLOC(max_str_len, char);
            ...
            /* conventional fcanf with "%s" only return the string before the first space character
             ...
             ,*/
                                rc2 = sscanf(line, " %1023[^\n]%n", buffer, &nread);
                               printf(":Dline6:%s:Fline6:\n", line);
                                printf("==> Mot détecté (buffer6): %s, %ld\n\n", buffer, strlen(buffer));
                                line += nread;
                                printf(":Dline6-bis:%s:Fline6-bis:\n", line);
...
printf("==> Traitement de '$group_num$_isSet'\n");
        ...
        /* additional parameter $group_num$_isSet is needed to suppress warning when fscanf into bool variable using %u or %d */
        	rc2 = sscanf(line, "%u\n%n", &($group_num$_isSet), &nread);
printf(":Dline7:%s:Fline7:\n", line);
printf("==> Résultat sscanf : rc2 = %d\n", rc2);
printf("==> Valeur extraite : $group_num$_isSet = %u\n", $group_num$_isSet);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
        //      printf("==> Résultat: %s, %u\n\n", buffer,$group_num$_isSet);
                line += nread;
                printf(":Dline7-bis:%s:Fline7-bis:\n", line);
        	...
        	if ($group$->$group_num$_isSet == true) {
          		rc2 = sscanf(line, "%1023s%n", buffer, &nread);
printf(":Dline8:%s:Fline8:\n", line);
                        printf("==> Mot détecté (buffer8): %s, %ld\n\n", buffer, strlen(buffer));
                        line += nread;
                        printf(":Dline8-bis:%s:Fline8-bis:\n", line);
                                if ((rc2 != 1) || (strcmp(buffer, "$group_num$") != 0)) {
                                        printf("==> Traitement de '$group_num$'\n");
                                        trexio_s3_free_read_$group$(buffer, f, file, $group$);
                                        return NULL;
                                }
                                rc2 = sscanf(line, "%$group_num_format_scanf$%n", &($group$->$group_num$), &nread);
                                printf(":Dline9:%s:Fline9:\n", line);
printf("==> Résultat sscanf : rc2 = %d\n", rc2);
printf("==> Valeur extraite : $group$->$group_num$ = %$group_num_format_scanf$\n", $group$->$group_num$);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
//printf("==> Résultat: %s=%$group_num_format_scanf$\n", buffer, $group$->$group_num$);
                                line += nread;
                                printf(":Dline9-bis:%s:Fline9-bis:\n", line);
                        if (rc2 != 1) {
                                trexio_s3_free_read_$group$(buffer, f, file, $group$);
                                return NULL;
...
// END REPEAT GROUP_NUM
        // START REPEAT GROUP_ATTR_STR
                ...
                printf("==> Traitement de 'len_$group_str$'\n");
                rc2 = sscanf(line, "%" SCNu64"\n%n", &($group$->len_$group_str$), &nread);
                printf(":Dline10:%s:Fline10:\n", line);
        //      printf("==>Résultat: %s=%" SCNu64 "\n%n", buffer, $group$->len_$group_str$);
                line += nread;
                printf(":Dline10-bis:%s:Fline10-bis:\n", line);
                if (rc2 != 1) {
                        trexio_s3_free_read_$group$(buffer, f, file, $group$);
                        return NULL;
                }
                rc2 = sscanf(line, "%1023s%n", buffer, &nread);
                printf(":Dline11:%s:Fline11:\n", line);
                printf("==> Mot détecté (buffer11) : %s, %ld\n\n", buffer, strlen(buffer));
                line += nread;
                printf(":Dline11-bis:%s:Fline11-bis:\n", line);
                if ((rc2 != 1) || (strcmp(buffer, "$group_str$") != 0)) {
                        printf("==> Traitement de '$group_str$'\n");
                        trexio_s3_free_read_$group$(buffer, f, file, $group$);
                        return NULL;
                }
                if ($group$->len_$group_str$ != 0) {
                        ...
                        rc2 = sscanf(line, " %1023[^\n]%n", buffer, &nread);
                        printf(":Dline12:%s:Fline12:\n", line);
                        printf("==> Mot détecté (buffer12): %s, %ld\n\n", buffer, strlen(buffer));
                        line += nread;
                        printf(":Dline12-bis:%s:Fline12-bis:\n", line);
                        if (rc2 != 1) {
                                trexio_s3_free_read_$group$(buffer, f, file, $group$);
                                return NULL;
                        }
        ...
        rc2 = sscanf(line, "%1023s%n", buffer, &nread);
        printf(":Dline13:%s:Fline13:\n", line);
        printf("==> Mot détecté (buffer13): %s, %ld\n\n", buffer, strlen(buffer));
//        line += nread;
        printf(":Dline13-bis:%s:Fline13-bis:\n", line);
    }
...
{
printf("===== Début de trexio_s3_has_group ======\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
{
printf("======== Début de trexio_s3_fulsh_group ========\n");
  if (file == NULL) return TREXIO_INVALID_ARG_1;
...
printf("[FLUSH8] %s", line);
  ...
  for (uint64_t i=0 ; i<size_$group_dset$ ; ++i) {
//    printf("$group$->$group_dset$[i]: %$group_dset_format_printf$\n");
    snprintf(line, sizeof(line), "%$group_dset_format_printf$\n", $group$->$group_dset$[i]);
printf("[FLUSH9] %s", line);
    line_len=strlen(line);
    // === Réallocation si nécessaire ===
    while(buffer_size + line_len >= buffer_init){
      buffer_init *=2;
...
printf("rc_s3_put_in_upload_final_to_S3_trexio_backend_S3 = %d\n", rc);//////////////
  FREE(buffer);
...
{
printf("======== Debut de trexio_s3_read_group_num =====\n");
...
{
printf("=========Début de trexio_s3_read_group_dset ========\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
trexio_s3_write_$group_dset$ (trexio_t* const file, const $group_dset_dtype$* $group_dset$,
                                const uint32_t rank, const uint64_t* dims)
{
printf("==== Début de trexio_s3_group_dset =====\n");
//  if (file  == NULL)  return TREXIO_INVALID_ARG_1;
  if (file  == NULL){
	printf("Erreur1 : File ==NULL\n");
	  return TREXIO_INVALID_ARG_1;
  }

//  if ($group_dset$ == NULL)  return TREXIO_INVALID_ARG_2;
  if ($group_dset$ == NULL){
	printf("Erreur2: $group_dset$==NULL\n");
	return TREXIO_INVALID_ARG_2;
  }
//  if (rank < 1) return TREXIO_INVALID_ARG_3;
  if (rank < 1){
	printf("Erreur3: rank<1 (%u)\n", rank);
	return TREXIO_INVALID_ARG_3;
  }
//  if (dims == NULL) return TREXIO_INVALID_ARG_4;
  if (dims == NULL){
	printf("Erreur4: dims == NULL\n");
	return TREXIO_INVALID_ARG_4;
  }

//  if (file->mode == 'r') return TREXIO_READONLY;
  if (file->mode == 'r'){
	printf("Erreur5: fichier en mode lecture uniquement\n");
...
//  if ($group$ == NULL) return TREXIO_FAILURE;
  ...
  if ($group$ == NULL) {
    printf("Erreur7: $group$ == NULL après trexio_s3_read_$group$\n");
    return TREXIO_FAILURE;
  }

  if ($group$->$group_dset$ != NULL) {
    printf("Libération de l'ancien tableau %s\n", "$group_dset$");
    FREE($group$->$group_dset$);
  }
printf("Initialisation de rank = %u\n", rank);
  ...
  for (uint32_t i=0; i<$group$->rank_$group_dset$; ++i){
    ...
    dim_size *= dims[i];
printf("dims[%u] = %" PRIu64 "\n", i, dims[i]);
  }
printf("Taille totale à allouer = %" PRIu64 "\n", dim_size);

  $group$->$group_dset$ = CALLOC(dim_size, $group_dset_dtype$);

 if ($group$->$group_dset$ == NULL) {                           //
    printf("Erreur d'allocation mémoire pour $group_dset$\n");  //
    return TREXIO_FAILURE;                                      //
  }                                                             //

  for (uint64_t i=0 ; i<dim_size ; ++i) {
    $group$->$group_dset$[i] = $group_dset$[i];
printf("Copie %s[%" PRIu64 "] = ...\n", "$group_dset$", i);  // Ajoute une valeur réelle si possible
  }
printf("Marquage to_flush = 1 pour $group$\n");
  $group$->to_flush = 1;

printf("Écriture dans $group$ terminée avec succès.\n");
  return TREXIO_SUCCESS;
...
printf("=========Début de trexio_s3_has_group_dset ========\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
printf("=========Début de trexio_s3_read_group_dset dataset of strings ========\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
  if (dset == NULL) return TREXIO_INVALID_ARG_2;

  $group$_t* const $group$ = trexio_s3_read_$group$((trexio_s3_t*) file);
  if ($group$ == NULL) return TREXIO_FAILURE;
printf("[DEBUG1] rank demandé: %u, rank enregistré: %u\n", rank, $group$->rank_$group_dset$);

  if (rank != $group$->rank_$group_dset$){
printf("[ERROR] Rank mismatch: attendu %u, trouvé %u\n", rank, $group$->rank_$group_dset$);
return TREXIO_INVALID_ARG_3;
}
  for (uint32_t i=0 ; i<rank ; ++i) {
printf("[DEBUG2] dims[%u] = %" PRIu64 ", dims_in_file[%u] = %" PRIu64 "\n",
           i, dims[i], i, $group$->dims_$group_dset$[i]);

    if (dims[i] != $group$->dims_$group_dset$[i]){ 
      printf("[ERROR] Dimension mismatch at dim %u\n", i);
      return TREXIO_INVALID_ARG_4;
    }

   }
  printf("[DEBUG3] max_str_len = %u\n", max_str_len);
  printf("[DEBUG4] Lecture des chaînes de caractères...\n");


  strcpy(dset, "");
printf("[DEBUG4.1] Adresse du tableau de chaînes: %p\n", (void*) $group$->$group_dset$);
if ($group$->$group_dset$ == NULL) {
    printf("[ERREUR] $group_dset$ est toujours NULL !\n");
    return TREXIO_FAILURE; // On sort proprement
}
  for (uint64_t i=0 ; i<dims[0] ; ++i) {
 printf("[DEBUG5] i = %" PRIu64 ", label = '%s'\n", i, $group$->$group_dset$[i]);
    strncat(dset, $group$->$group_dset$[i], max_str_len);
    strcat(dset, TREXIO_DELIM);
  }
  printf("[DEBUG6] Contenu final dset = \"%s\"\n", dset);
  printf("=== [DEBUG7] Sortie de trexio_s3_read_$group_dset$ ===\n");
...
printf("=========Début de trexio_s3_write_group_dset dataset of strings ========\n");
 if (file  == NULL)  return TREXIO_INVALID_ARG_1;
  if (dset == NULL)  return TREXIO_INVALID_ARG_2;

  if (file->mode == 'r') return TREXIO_READONLY;
printf("[INFO]  Ecriture ou allocation du $group_dset$\n");
printf("[INFO] Lecture ou allocation du groupe $group$\n");

  $group$_t* const $group$ = trexio_s3_read_$group$((trexio_s3_t*) file);
  if ($group$ == NULL) return TREXIO_FAILURE;

  if ($group$->$group_dset$ != NULL) {
printf("[INFO] Libération de l'ancien tableau $group_dset$\n");
...
printf("[INFO] Rank = %u\n", rank);
  ...
  for (uint32_t i=0; i<$group$->rank_$group_dset$; ++i){
    $group$->dims_$group_dset$[i] = dims[i];
printf("[INFO] dims[%u] = %" PRIu64 "\n", i, dims[i]);
  }

  $group$->$group_dset$ = CALLOC(dims[0], char*);
  if ($group$->$group_dset$ == NULL){
    printf("[ERREUR] Allocation échouée pour $group_dset$\n");
    return TREXIO_ALLOCATION_FAILED;
  }
size_t total_buffer = dims[0]*32 + 1;
  char* tmp_str = CALLOC(total_buffer, char);
  if (tmp_str == NULL){
    printf("[ERREUR] Allocation échouée pour le buffer temporaire de %zu octets\n", total_buffer);
    return TREXIO_ALLOCATION_FAILED;
  }
  printf("[INFO] Allocation du buffer temporaire réussie (%zu octets)\n", total_buffer);
  for (uint64_t i=0 ; i<dims[0] ; ++i) {
    size_t tmp_len = strlen(dset[i])+1;
printf("[INFO] dset[%" PRIu64 "] = '%s' (longueur = %zu)\n", i, dset[i], tmp_len);
  ...
  $group$->to_flush = 1;
printf("[INFO] Marquage du groupe $group$ pour flush (to_flush = 1)\n");
printf("[SUCCESS] Écriture dans $group_dset$ terminée avec succès\n");
...
printf("=========Début de trexio_s3_has_group_dset dataset of strings ========\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
printf("=========Début de trexio_s3_read_group_dset string attribute  ========\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
printf("=========Début de trexio_s3_write_group_dset string attribute  ========\n");
  if (file == NULL) return TREXIO_INVALID_ARG_1;
  if (str  == NULL) return TREXIO_INVALID_ARG_2;
...
printf("=========Début de trexio_s3_has_group_dset string attribute  ========\n");
  if (file  == NULL) return TREXIO_INVALID_ARG_1;
...
trexio_exit_code trexio_s3_read_$group_dset$(trexio_t* const file,
                                               ...
                                               double* const dset)
{
printf("======= Début trexio_s3_read_group_dset buffered vector =====\n");
  if (file == NULL) return TREXIO_INVALID_ARG_1;
...
{
printf("======= Début trexio_s3_read_group_dset_size buffered vector =====\n");
  if (file == NULL) return TREXIO_INVALID_ARG_1;
...
trexio_exit_code trexio_s3_write_$group_dset$(trexio_t* const file,
                                                ...
                                                const double* dset)
{
printf("======= Début trexio_s3_write_group_dset buffered vector =====\n");
  if (file == NULL) return TREXIO_INVALID_ARG_1;
  if (offset_file < 0) return TREXIO_INVALID_ARG_2;
  if (rank < 1) return TREXIO_INVALID_ARG_3;
  if (dims == NULL) return TREXIO_INVALID_ARG_4;
...
trexio_exit_code trexio_s3_has_$group_dset$(trexio_t* const file)
{
printf("======= Début trexio_s3_has_group_dset buffered vector =====\n");
  if (file == NULL) return TREXIO_INVALID_ARG_1;

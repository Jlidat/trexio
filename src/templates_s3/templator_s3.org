...
////printf("rc_s3_get_in_read_group_trexio_s3_backend=%d\n",rc);///////////////
    if(rc != 0){
      FREE(f);
      FREE($group$);
      return NULL;
	...
	if (strcmp(buffer, "EXIT") == 0) {
		...
		printf("==> Résultat sscanf : rc = %d\n", rc);
		printf("==> Valeur extraite : rank = %u\n", $group$->rank_$group_dset$);
		printf("==> Nombre de caractères lus : nread = %d\n", nread);
		printf("==> Mot détecté (buffer1 toujours): %s, %ld\n\n", buffer, strlen(buffer));
...
printf("==> Résultat sscanf : rc = %d\n", rc);
printf("==> Valeur extraite : dims = %" PRIu64 "\n", $group$->dims_$group_dset$[i]);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
			printf("==> Mot détecté (buffer4): %s, %ld\n\n", buffer, strlen(buffer));
        ...
        /* Allocate arrays */
        	...
        	for (uint64_t i=0 ; i<size_$group_dset$ ; ++i) {
          		rc = sscanf(line, "%$group_dset_format_scanf$%n", &($group$->$group_dset$[i]), &nread);
	  		printf(":Dline5:%s:Fline5:\n", line);
printf("==> Résultat sscanf : rc = %d\n", rc);
//printf("==> Valeur extraite : $group$->$group_dset$[%ld] = %PRId64\n", i, $group$->$group_dset$[i]);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
			printf("==> Mot détecté (buffer5): %s, %ld\n\n", buffer, strlen(buffer));
	  		line += nread;
			printf(":Dline5:%s:Fline5:\n", line);
          		if (rc != 1) {
            			trexio_s3_free_read_$group$(buffer, f, file, $group$);
...
printf("==> Nombre de caractères lus : nread = %d\n", nread);
		printf("==> Mot détecté(buffer7): %s, %ld\n\n", buffer, strlen(buffer));
        	line += nread;
		printf(":Dline7-bis:%s:Fline7-bis:\n", line);
        	...
        	if ($group$->$group_num$_isSet == true) {
          		rc = sscanf(line, "%1023s%n", buffer, &nread);
			...
			printf(":Dline8-bis:%s:Fline8-bis:\n", line);
					...
					printf("==> Traitement de '$group_num$'\n");
          			...
          			rc = sscanf(line, "%$group_num_format_scanf$%n", &($group$->$group_num$), &nread);
				printf(":Dline9:%s:Fline9:\n", line);
printf("==> Résultat sscanf : rc = %d\n", rc);
printf("==> Valeur extraite : $group$->$group_num$ = %$group_num_format_scanf$\n", $group$->$group_num$);
printf("==> Nombre de caractères lus : nread = %d\n", nread);
				printf("==> Mot détecté (buffer9): %s, %ld\n\n", buffer, strlen(buffer));
          			line += nread;
		...
		printf("==> Traitement de 'len_$group_str$'\n");
        	rc = sscanf(line, "%" SCNu64 "%n", &($group$->len_$group_str$), &nread);
		printf(":Dline10:%s:Fline10:\n", line);
		printf("==> Mot détecté (buffer10): %s, %ld\n\n", buffer, strlen(buffer));
        	line += nread;
...
/////printf("!!!!!!!!rc_trexio_s3_flush_group_in_has_group=%d\n",rc);
...
//=== Fin du fichier ==
  ...
  line_len=strlen(line)+1;  // +1 to include terminating '\0'
  // === Réallocation si nécessaire ===
  while(buffer_size + line_len >= buffer_init){
    buffer_init *=2;
    char* new_buffer=realloc(buffer, buffer_init*sizeof(char));
